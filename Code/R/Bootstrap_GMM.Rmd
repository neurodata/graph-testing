...
For simplicity, we assume each population has a Gaussian Mixture Model distribution with variance 1. As a two-sample problem, we test the null hypothesis that

Ho: $F=G$ via Helligner_distance(F, G) = 1 (test statistic)

The bootstrap framework follows [BootStrap_SingleGaussian]('')
...
###Assumptions:
$F$ ~ $\pi * N(\mu_{1}, \sigma^{2}) + (1-\pi)* N(\mu_{2}, \sigma^{2})$

$G$ ~ $\pi * N(\mu_{3}, \sigma^{2}) + (1-\pi)* N(\mu_{4}, \sigma^{2})$
